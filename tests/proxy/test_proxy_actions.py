...
class TestActionMethods(unittest.TestCase):
    ...
    def test_create_proxy_from_env_success(self, toxiproxyapi_mock, mockconfig):
        ...
        self.assertTrue(
            actions.create_proxy(
                proxy_name="createproxy",
                listen_port=0,
                upstream_host="env:RANDOM_HOST_KEY",
                upstream_port="env:RANDOM_PORT_KEY",
                configuration=mockconfig,
            )
        )
        # the action has the side effect of adding the proxy port into the config with the key proxyname_PORT
    ...
    def test_create_proxy_from_env_failure(self, toxiproxyapi_mock, mockconfig):
        ...
        with self.assertRaises(KeyError):
            actions.create_proxy(
                proxy_name="aproxy",
                listen_port=0,
                upstream_host="env:DONT_EXISTS",
                upstream_port="env:DONT_EXISTS",
                configuration=mockconfig,
            )
    ...
    def test_modify_proxy_everything(self, toxiproxyapi_mock, mockconfig):
        ...
        self.assertTrue(
            actions.modify_proxy(
                proxy_name="change_all",
                listen_address="127.0.0.1:6666",
                upstream_address="10.10.10.10:1234",
                enabled=True,
                configuration=mockconfig,
            )
        )
        toxiproxyapi_mock.assert_called_once_with(
            proxy_name="change_all", proxy_json=modify_json, configuration=mockconfig
        )
        # the action has the side effect of adding the proxy port into the config with the key proxyname_PORT
    ...
    def test_modify_proxy_listen(self, toxiproxyapi_mock, mockconfig):
        ...
        self.assertTrue(
            actions.modify_proxy(
                proxy_name="change_one",
                listen_address="0.0.0.0:43409",
                configuration=mockconfig,
            )
        )
        toxiproxyapi_mock.assert_called_once_with(
            proxy_name="change_one", proxy_json=modify_json, configuration=mockconfig
        )
        # the action has the side effect of adding the proxy port into the config with the key proxyname_PORT
    ...
    def test_modify_proxy_upstream(self, toxiproxyapi_mock, mockconfig):
        ...
        self.assertTrue(
            actions.modify_proxy(
                proxy_name="change_two",
                upstream_address="10.10.10.10:1234",
                configuration=mockconfig,
            )
        )
        toxiproxyapi_mock.assert_called_once_with(
            proxy_name="change_two", proxy_json=modify_json, configuration=mockconfig
        )
        # the action has the side effect of adding the proxy port into the config with the key proxyname_PORT
    ...
    def test_enable_proxy(self, toxiproxyapi_mock, mockconfig):
        ...
        self.assertTrue(
            actions.enable_proxy(proxy_name="enableproxy", configuration=mockconfig)
        )
        toxiproxyapi_mock.assert_called_once_with(
            proxy_name="enableproxy", proxy_json=modify_json, configuration=mockconfig
        )
        # the action has the side effect of adding the proxy port into the config with the key proxyname_PORT
    ...
    def test_disable_proxy(self, toxiproxyapi_mock, mockconfig):
        ...
        self.assertTrue(
            actions.disable_proxy(proxy_name="disable_proxy", configuration=mockconfig)
        )
        toxiproxyapi_mock.assert_called_once_with(
            proxy_name="disable_proxy", proxy_json=modify_json, configuration=mockconfig
        )
        # the action has the side effect of adding the proxy port into the config with the key proxyname_PORT
